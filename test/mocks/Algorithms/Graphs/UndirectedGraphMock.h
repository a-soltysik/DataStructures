...
public:
    ...
    MOCK_METHOD(bool, RemoveVertex, (Vertex), (override));
...
std::ostream& operator<<(std::ostream& os, const UndirectedGraph::EdgeData& edge)
{
    os << "v: " << edge.vertices.first << ", u: " << edge.vertices.second << ", weight: " << edge.weight;
    return os;
}

std::ostream& operator<<(std::ostream& os, const Graph::Neighbour& neighbour)
{
